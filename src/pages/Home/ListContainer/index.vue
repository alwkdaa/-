...
export default {
  ...
  mounted() {
    ...
    // 在 new Swiper实例之前，页面中结构必须得有
    // 直接将new Swiper放在mounted中不行，因为dispatch当中涉及到异步语句，导致v-for遍历的时候结构还没有完全，因此不行
    // 方法一：使用延时器
    /* setTimeout(() => {
  ...
  /* watch: {
    immediate: true,
    // 监听bannerList数据的变化:因为这条数据发生过变化---由空函数变为数组里面由四个元素
    bannerList: {
      handler(newValue, oldValue) {
        ...
        // v-for执行完毕,才有结构[现在在watch中没办法保证]
        // nextTick:在下次DOM更新  循环结束之后 执行延迟回调,在  修改数据之后 立即使用这个方法,获取更新后的DOM
        this.$nextTick(() => {
          // 当执行这个回调的时候:保证服务器数据回来了,v-for执行完毕了[一定轮播图的结构一定有了]
